const alphabet = {
  "default": { 
  //   "offsetx": 0, //position x of ellipse
  //   "offsety": 0, //position y of ellipse

  //   "Trix1": -32, //triangle left corner x
  //   "Triy1":  50, //triangle left corner y
  //   "Trix2":  0,  //triangle top corner x
  //   "Triy2": -50,  //triangle top corner y
  //   "Trix3": 29, //triangle right corner x
  //   "Triy3": 50, //triangle right corner y

  // "arcx": 0, //x pos of arc
  // "arcy": 0, //y pos pf arc
  // "arcStart": 183.6, //start of arc
  // "arcStop": 180, //end of arc
  
  // "linex": 0, //top x of line
  // "liney": 0, //top y of line
  // "linex2": 0, //bottom x of line
  // "liney2": 0, //bottom y of line

  //option 1 
  "offsetx": -0.6, //position x of ellipse
  "offsety": -40, //position y of ellipse

  "Trix1": -18, //triangle left corner x
  "Triy1": 0, //triangle left corner y
  "Trix2": 0, //triangle top corner x
  "Triy2": 36, //triangle top corner y
  "Trix3": 16, //triangle right corner x
  "Triy3": 0, //triangle right corner y

  "arcx": 0, //x pos of arc
  "arcy": -2, //y pos of arc
  "arcStart": 230.4, //start of arc
  "arcStop": 313.2, //end of arc

  "linex": 0, //top x of line
  "liney": -60, //top y of line
  "linex2": 0, //bottom x of line
  "liney2": 42, //bottom y of line

  //option 2
 

  },
  "A": {
    "offsetx": -10, //position x of ellipse
    "offsety": 0, //position y of ellipse

    "Trix1": 0, //triangle left corner x
  "Triy1": 50, //triangle left corner y
  "Trix2": 20, //triangle top corner x
  "Triy2": -10, //triangle top corner y
  "Trix3": 40, //triangle right corner x
  "Triy3": 50, //triangle right corner y

  "arcx": -5, //x pos of arc
  "arcy": 0, //y pos pf arc
  "arcStart": 61.2, //start of arc
  "arcStop": 298.8, //end of arc
  
  "linex": 30, //top x of line
  "liney": -10, //top y of line
  "linex2": 50, //bottom x of line
  "liney2": 48, //bottom y of line

  },
  "B": {
    
    "offsetx": 10, //position x of ellipse
    "offsety": 0, //position y of ellipse

    "Trix1": -40, //triangle left corner x
    "Triy1": 50, //triangle left corner y
  "Trix2": -20, //triangle top corner x
  "Triy2": -150, //triangle top corner y
  "Trix3": 0, //triangle right corner x
  "Triy3": 50, //triangle right corner y

  "arcx": 5, //x pos of arc
  "arcy": 0, //y pos pf arc
  "arcStart": 237.6, //start of arc
  "arcStop": 126, //end of arc

  "linex": -30, //top x of line
  "liney": -145, //top y of line
  "linex2": -50, //bottom x of line
  "liney2": 48, //bottom y of line

  },
  "C": {
    
    "offsetx": 30, //position x of ellipse
    "offsety": -35, //position y of ellipse

    "Trix1": 0, //triangle left corner x
    "Triy1": 0, //triangle left corner y
  "Trix2": 0, //triangle top corner x
  "Triy2": 0, //triangle top corner y
  "Trix3": 0, //triangle right corner x
  "Triy3": 0, //triangle right corner y

  "arcx": 5, //x pos of arc
  "arcy": 0, //y pos pf arc
  "arcStart": 61.2, //start of arc
  "arcStop": 266.4, //end of arc

  "linex": -5, //top x of line
  "liney": -10, //top y of line
  "linex2": -5, //bottom x of line
  "liney2": 10, //bottom y of line

  },
  "D": {
    
    "offsetx": -10, //position x of ellipse
    "offsety": 0, //position y of ellipse

    "Trix1": 0, //triangle left corner x
    "Triy1": 50, //triangle left corner y
  "Trix2": 20, //triangle top corner x
  "Triy2": -150, //triangle top corner y
  "Trix3": 40, //triangle right corner x
  "Triy3": 50, //triangle right corner y

  "arcx": -5, //x pos of arc
  "arcy": 0, //y pos pf arc
  "arcStart": 61.2, //start of arc
  "arcStop": 298.8, //end of arc

  "linex": 30, //top x of line
  "liney": -145, //top y of line
  "linex2": 50, //bottom x of line
  "liney2": 48, //bottom y of line
  
  },
  "E": {

   
    "offsetx": 0, //position x of ellipse
    "offsety": -15, //position y of ellipse

    "Trix1": 0, //triangle left corner x
    "Triy1": 0, //triangle left corner y
    "Trix2": 0, //triangle top corner x
    "Triy2": 0, //triangle top corner y
    "Trix3": 0, //triangle right corner x
    "Triy3": 0, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 60.6, //start of arc
    "arcStop": 310.2, //stop of arc

    "linex": 25, //top x of line
  "liney": 0, //top y of line
  "linex2": -10, //bottom x of line
  "liney2": 0, //bottom y of line
    
  },
  "F": {

    "offsetx": 11.4, //position x of ellipse
    "offsety": -50, //position y of ellipse

    "Trix1": -40, //triangle left corner x 
    "Triy1": 50, //triangle left corner y 
    "Trix2": -26, //triangle top corner x 
    "Triy2": -84, //triangle top corner y 
    "Trix3": -12, //triangle right corner x
    "Triy3": 50, //triangle right corner y

    "arcx": 10, //x pos of arc
    "arcy": -100, //y pos of arc
    "arcStart": 183.6, //bottom x of line
    "arcStop": 324, //bottom y of line

    "linex": -44, //top x of line
    "liney": -50, //top y of line
    "linex2": -2, //bottom x of line
    "liney2": -50, //bottom y of line
    
  },
  "G": {
    
    "offsetx": -10, //position x of ellipse
    "offsety": 0, //position y of ellipse

    "Trix1": 0, //triangle left corner x
    "Triy1": -50, //triangle left corner y
  "Trix2": 20, //triangle top corner x
  "Triy2": 120, //triangle top corner y
  "Trix3": 40, //triangle right corner x
  "Triy3": -50, //triangle right corner y

  "arcx": -5, //x pos of arc
  "arcy": 0, //y pos pf arc
  "arcStart": 61.2, //start of arc
  "arcStop": 298.8, //end of arc

  "linex": -15, //top x of line
  "liney": 95, //top y of line
  "linex2": 15, //bottom x of line
  "liney2": 115, //bottom y of line

  },
  "H": {

    "offsetx": 40, //position x of ellipse
    "offsety": 15, //position y of ellipse

    "Trix1": 50, //triangle left corner x
    "Triy1": 50, //triangle left corner y
    "Trix2": 40, //triangle top corner x
    "Triy2": 30, //triangle top corner y
    "Trix3": 30, //triangle right corner x
    "Triy3": 50, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 252, //start of arc
  "arcStop": 345.6, //end of arc
    
    "linex":- 35, //top x of line
    "liney": -145, //top y of line
    "linex2": -35, //bottom x of line
    "liney2": 48, //bottom y of line
   
  },
  "I": {

    "offsetx": -5, //position x of ellipse
    "offsety": -80, //position y of ellipse

    "Trix1": -37, //triangle left corner x
    "Triy1":  50, //triangle left corner y
    "Trix2":  -5,  //triangle top corner x
    "Triy2": -50,  //triangle top corner y
    "Trix3": 29, //triangle right corner x
    "Triy3": 50, //triangle right corner y

    "arcx": 18, //x pos of arc
    "arcy": -100, //y pos of arc
    "arcStart": 0, //start of arc
    "arcStop": 0, //end of arc
    
    "linex": 35, //top x of line
    "liney": 48, //top y of line
    "linex2": 5,  //bottom x of line
    "liney2": -48,  //bottom y of line

  },
  "J": {

    "offsetx": 0, //position x of ellipse
    "offsety": -80, //position y of ellipse

    "Trix1": -10, //triangle left corner x 
    "Triy1": -50, //triangle left corner y 
    "Trix2": 0, //triangle top corner x 
    "Triy2": 50, //triangle top corner y 
    "Trix3": 10, //triangle right corner x
    "Triy3": -50, //triangle right corner y

    "arcx": -35, //x pos of arc
    "arcy": 60, //y pos of arc
    "arcStart": 360, //bottom x of line
    "arcStop": 134, //bottom y of line
    
    "linex": 20, //top x of line
    "liney": -48, //top y of line
    "linex2": 10, //bottom x of line
    "liney2": 48, //bottom y of line
    
  },
  "K": {

    "offsetx": 15, //position x of ellipse
    "offsety": -5, //position y of ellipse

    "Trix1": -40, //triangle left corner x
    "Triy1": 50, //triangle left corner y
  "Trix2": -20, //triangle top corner x
  "Triy2": -150, //triangle top corner y
  "Trix3": 0, //triangle right corner x
  "Triy3": 50, //triangle right corner y

  "arcx": 25, //x pos of arc
  "arcy":-5, //y pos pf arc
  "arcStart": 61.2, //start of arc
  "arcStop": 298.8, //end of arc

  "linex": -30, //top x of line
  "liney": -148, //top y of line
  "linex2": -50, //bottom x of line
  "liney2": 48, //bottom y of line
   
  },
  "L": {

    "offsetx": -25, //position x of ellipse
    "offsety": -30, //position y of ellipse

    "Trix1": -35, //triangle left corner x
    "Triy1": -140, //triangle left corner y
    "Trix2": -25, //triangle top corner x
    "Triy2": -45, //triangle top corner y
    "Trix3": -15, //triangle right corner x
    "Triy3": -140, //triangle right corner y

  "arcx": 10, //x pos of arc
  "arcy": 0, //y pos pf arc
  "arcStart": 68, //start of arc
  "arcStop": 190, //end of arc

  "linex": -30, //top x of line
  "liney": -55, //top y of line
  "linex2": -40, //bottom x of line
  "liney2": -135, //bottom y of line
   
  },
  "M": {

  //   "offsetx": 0, //position x of ellipse
  //   "offsety": -10, //position y of ellipse
   
  //   "Trix1": -45, //triangle left corner x
  //   "Triy1": -45, //triangle left corner y
  //   "Trix2": -10, //triangle top corner x
  //   "Triy2": -15, //triangle top corner y
  //   "Trix3": -45, //triangle right corner x
  //   "Triy3": 50, //triangle right corner y

  //   "arcx": 10, //x pos of arc
  //   "arcy": 0, //y pos pf arc
  //   "arcStart": 280.8, //start of arc
  // "arcStop": 3.6, //end of arc

  // "linex": 48, //top x of line
  // "liney": 20, //top y of line
  // "linex2": 48, //bottom x of line
  // "liney2": 48, //bottom y of line

  "offsetx": 0, //position x of ellipse
    "offsety": -10, //position y of ellipse

  "Trix1": -50, //triangle left corner x
  "Triy1": -42, //triangle left corner y
  "Trix2": -36, //triangle top corner x
  "Triy2": 50, //triangle top corner y
  "Trix3": -18, //triangle right corner x
  "Triy3": -42, //triangle right corner y

    "arcx": 10, //x pos of arc 
    "arcy": 0, //y pos pf arc
    "arcStart": 280.8, //start of arc
  "arcStop": 3.6, //end of arc

  "linex": 48, //top x of line
  "liney": 20, //top y of line
  "linex2": 48, //bottom x of line
  "liney2": 48, //bottom y of line

  },
  "N": {
    
    "offsetx": 40, //position x of ellipse
    "offsety": 15, //position y of ellipse

    "Trix1": 50, //triangle left corner x
    "Triy1": 50, //triangle left corner y
    "Trix2": 40, //triangle top corner x
    "Triy2": 30, //triangle top corner y
    "Trix3": 30, //triangle right corner x
    "Triy3": 50, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 252, //start of arc
  "arcStop": 345.6, //end of arc
    
    "linex":- 35, //top x of line
    "liney": -50, //top y of line
    "linex2": -35, //bottom x of line
    "liney2": 48, //bottom y of line

  },
  "O": {

    "offsetx": 30, //position x of ellipse
    "offsety": -20, //position y of ellipse

    "Trix1": 40, //triangle left corner x
    "Triy1": 10, //triangle left corner y
    "Trix2": 30, //triangle top corner x
    "Triy2": -10, //triangle top corner y
    "Trix3": 20, //triangle right corner x
    "Triy3": 10, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 46.8, //start of arc
    "arcStop": 295.2, //stop of arc

    "linex": 0, //top x of line
  "liney": -10, //top y of line
  "linex2": 0, //bottom x of line
  "liney2": 10, //bottom y of line
   
  },
  "P": {
   
    "offsetx": 10, //position x of ellipse
    "offsety": 0, //position y of ellipse

    "Trix1": -40, //triangle left corner x
    "Triy1": -50, //triangle left corner y
  "Trix2": -20, //triangle top corner x
  "Triy2": 110, //triangle top corner y
  "Trix3": 0, //triangle right corner x
  "Triy3": -50, //triangle right corner y

  "arcx": 5, //x pos of arc
  "arcy": 0, //y pos pf arc
  "arcStart": 237.6, //start of arc
  "arcStop": 126, //end of arc

  "linex": -45, //top x of line
  "liney": -48, //top y of line
  "linex2": -25, //bottom x of line
  "liney2": 105, //bottom y of line 
    
  },
  "Q": {
    
    "offsetx": -10, //position x of ellipse
    "offsety": 0, //position y of ellipse

    "Trix1": 0, //triangle left corner x
    "Triy1": -50, //triangle left corner y
  "Trix2": 20, //triangle top corner x
  "Triy2": 120, //triangle top corner y
  "Trix3": 40, //triangle right corner x
  "Triy3": -50, //triangle right corner y

  "arcx": -5, //x pos of arc
  "arcy": 0, //y pos pf arc
  "arcStart": 61.2, //start of arc
  "arcStop": 298.8, //end of arc

  "linex": 55, //top x of line
  "liney": 95, //top y of line
  "linex2": 25, //bottom x of line
  "liney2": 115, //bottom y of line

   },
  "R": {

    "offsetx": 40, //position x of ellipse
    "offsety": -10, //position y of ellipse

    "Trix1": -40, //triangle left corner x
    "Triy1": 50, //triangle left corner y
    "Trix2": -20, //triangle top corner x
    "Triy2": -70, //triangle top corner y
    "Trix3": 0, //triangle right corner x
    "Triy3": 50, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 252, //start of arc
    "arcStop": 315, //end of arc
    
    "linex": -30, //top x of line
    "liney": -65, //top y of line
    "linex2": -50, //bottom x of line
    "liney2": 48, //bottom y of line
    
  },
  "S": {

    "offsetx": 10, //position x of ellipse
    "offsety": -20, //position y of ellipse

    "Trix1": 15, //triangle left corner x
    "Triy1": 50, //triangle left corner y
    "Trix2": -10, //triangle top corner x
    "Triy2": -10, //triangle top corner y
    "Trix3": 35, //triangle right corner x
    "Triy3": 15, //triangle right corner y

    "arcx": 20, //x pos of arc
    "arcy": -5, //y pos pf arc
    "arcStart": 210, //start of arc
    "arcStop": 300, //end of arc
    
    "linex": -30, //top x of line
    "liney": 33,  //top y of line
    "linex2": 5, //bottom x of line
    "liney2": 48, //bottom y of line
   
  
  },
  "T": {

    "offsetx": 35, //position x of ellipse
    "offsety": -50, //position y of ellipse

    "Trix1": -27, //triangle left corner x
    "Triy1":  50, //triangle left corner y
    "Trix2":  -5, //triangle top corner x
    "Triy2": -150, //triangle top corner y
    "Trix3": 20, //triangle right corner x
    "Triy3": 50, //triangle right corner y

    "arcx": 18, //x pos of arc
    "arcy": -100, //y pos of arc
    "arcStart": 0, //start of arc
    "arcStop": 0, //end of arc
    
    "linex": -40, //top x of line
    "liney": -50, //top y of line
    "linex2": 20, //bottom x of line
    "liney2": -50, //bottom y of line
    
  },
  "U": {
    
    "offsetx": -40, //position x of ellipse
    "offsety": -15, //position y of ellipse

    "Trix1": -50, //triangle left corner x
    "Triy1": -50, //triangle left corner y
    "Trix2": -40, //triangle top corner x
    "Triy2": -30, //triangle top corner y
    "Trix3": -30, //triangle right corner x
    "Triy3": -50, //triangle right corner y

    "arcx": -5, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 70, //start of arc
    "arcStop": 160, //end of arc

    "linex": 35, //top x of line
    "liney": -48, //top y of line
    "linex2": 35, //bottom x of line
    "liney2": 48, //bottom y of line

  },
  "V": {

    "offsetx": -5, //position x of ellipse
    "offsety": -20, //position y of ellipse

    "Trix1": -37, //triangle left corner x
    "Triy1":  -50, //triangle left corner y
    "Trix2":  -5,  //triangle top corner x
    "Triy2": 50,  //triangle top corner y
    "Trix3": 29,  //triangle right corner x
    "Triy3": -50,  //triangle right corner y

    "arcx": 18, //x pos of arc
    "arcy": -100, //y pos of arc
    "arcStart": 0, //start of arc
    "arcStop": 0, //end of arc
    
    "linex": 10, //top x of line
    "liney": 48, //top y of line
    "linex2": 40, //bottom x of line
    "liney2": -48, //bottom y of line
   
  },
  "W": {

  //   "offsetx": 0, //position x of ellipse
  //   "offsety": 0, //position y of ellipse
   
  //   "Trix1": -45, //triangle left corner x
  //   "Triy1": -45, //triangle left corner y
  //   "Trix2": -10, //triangle top corner x
  //   "Triy2": 20, //triangle top corner y
  //   "Trix3": -45, //triangle right corner x
  //   "Triy3": 50, //triangle right corner y

  //   "arcx": 12, //x pos of arc
  //   "arcy": 0, //y pos of arc
  //   "arcStart": 370, //bottom x of line
  //   "arcStop": 90, //bottom y of line

  // "linex": 48, //top x of line
  // "liney": -40, //top y of line
  // "linex2": 48, //bottom x of line
  // "liney2": -10, //bottom y of line

  "offsetx": 0, //position x of ellipse
  "offsety": 0, //position y of ellipse
 
  "Trix1": -50, //triangle left corner x
  "Triy1": 42, //triangle left corner y
  "Trix2": -36, //triangle top corner x
  "Triy2": -45, //triangle top corner y
  "Trix3": -18, //triangle right corner x
  "Triy3": 42, //triangle right corner y

  "arcx": 12, //x pos of arc
  "arcy": 0, //y pos of arc
  "arcStart": 370, //bottom x of line
  "arcStop": 90, //bottom y of line

"linex": 48, //top x of line
"liney": -40, //top y of line
"linex2": 48, //bottom x of line
"liney2": -10, //bottom y of line
    
  },
  "X": {
   
    "offsetx": 25, //25 //position x of ellipse
    "offsety": 0, //-130 //position y of ellipse

    "Trix1": -45, //triangle left corner x
    "Triy1": 0, //triangle left corner y
    "Trix2": -5, //triangle top corner x
    "Triy2": 0, //triangle top corner y
    "Trix3": -45, //triangle right corner x
    "Triy3": 40, //triangle right corner y

    "arcx": 40, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 97.2, //start of arc
    "arcStop": 266.4, //end of arc

    "linex": -40, //top x of line
    "liney": -40, //top y of line
    "linex2": -7, //bottom x of line
    "liney2": -5, //bottom y of line

  },
  "Y": {

    "offsetx": -35, //position x of ellipse
    "offsety": -40, //position y of ellipse

    "Trix1": 0, //triangle left corner x
    "Triy1": -50, //triangle left corner y
  "Trix2": 20, //triangle top corner x
  "Triy2": 110, //triangle top corner y
  "Trix3": 40, //triangle right corner x
  "Triy3": -50, //triangle right corner y

  "arcx": 0, //x pos of arc
  "arcy": 0, //y pos pf arc
  "arcStart": 68, //start of arc
  "arcStop": 190, //end of arc

  "linex": -10, //top x of line
  "liney": 100, //top y of line
  "linex2": 20, //bottom x of line
  "liney2": 120, //bottom y of line
   
  },
  "Z": {

    // "offsetx": 0, //position x of ellipse
    // "offsety": -35,  //position y of ellipse

    // "Trix1": -30, //triangle left corner x
    // "Triy1":  50, //triangle left corner y
    // "Trix2":  30, //triangle top corner x
    // "Triy2": -40, //triangle top corner y
    // "Trix3": 50, //triangle right corner x
    // "Triy3": 50, //triangle right corner y

    // "arcx": 0, //x pos of arc
    // "arcy": 0, //y pos pf arc
    // "arcStart": 0, //start of arc
    // "arcStop": 0, //end of arc

    // "linex": -30, //top x of line
    // "liney": -50, //top y of line
    // "linex2": 30, //bottom x of line
    // "liney2": -50, //bottom y of line

    "offsetx": 15,  //position x of ellipse
    "offsety": -35,  //position y of ellipse

    "Trix1": -40, //triangle left corner x
    "Triy1":  50, //triangle left corner y
    "Trix2":  10, //triangle top corner x
    "Triy2": -25, //triangle top corner y
    "Trix3": 50, //triangle right corner x
    "Triy3": 50, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 0, //start of arc
    "arcStop": 0, //end of arc

    "linex": -40, //top x of line
    "liney": -50, //top y of line
    "linex2": 20, //bottom x of line
    "liney2": -50, //bottom y of line
    
  },
  "0": {

    // "offsetx": 0, //position x of ellipse
    // "offsety": -130, //position y of ellipse

    // "Trix1": -36, //triangle left corner x
    // "Triy1": -30, //triangle left corner y
    // "Trix2": 36, //triangle top corner x
    // "Triy2": -120, //triangle top corner y
    // "Trix3": 36, //triangle right corner x
    // "Triy3": -30, //triangle right corner y

    // "arcx": 0, //x pos of arc
    // "arcy": 0, //y pos of arc
    // "arcStart": 360, //start of arc
    // "arcStop": 176.4, //stop of arc
    
    // "linex": -36, //top x of line
    // "liney": -110,  //top y of line
    // "linex2": -36, //bottom x of line
    // "liney2": -50, //bottom y of line

    "offsetx": 0, //position x of ellipse
    "offsety": -50, //position y of ellipse

    "Trix1": -30, //triangle left corner x
    "Triy1": -140, //triangle left corner y
    "Trix2": 36, //triangle top corner x
    "Triy2": -30, //triangle top corner y
    "Trix3": 36, //triangle right corner x
    "Triy3": -140, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos of arc
    "arcStart": 360, //start of arc
    "arcStop": 176.4, //stop of arc
    
    "linex": -36, //top x of line
    "liney": -110,  //top y of line
    "linex2": -36, //bottom x of line
    "liney2": -30, //bottom y of line
    
    
  },
  "1": {

    "offsetx": 0, //position x of ellipse
    "offsety": -135, //position y of ellipse


    "Trix1": -34, //triangle left corner x
    "Triy1":  40, //triangle left corner y
    "Trix2":  0, //triangle top corner x
    "Triy2": -120, //triangle top corner y
    "Trix3": 34, //triangle right corner x
    "Triy3": 40, //triangle right corner y

    "arcx": 18, //x pos of arc
    "arcy": -100, //y pos of arc
    "arcStart": 0, //start of arc
    "arcStop": 0, //end of arc
    
    "linex": -10, //top x of line
    "liney": -125, //top y of line
    "linex2": -40, //bottom x of line
    "liney2": -105, //bottom y of line

    
  },
  "2": {

  //   "offsetx": -10, //position x of ellipse
  //   "offsety": -100, //position y of ellipse

  //   "Trix1": -44, //triangle left corner x
  //   "Triy1": 34, //triangle left corner y
  //   "Trix2": 40, //triangle top corner x
  //   "Triy2": 34, //triangle top corner y
  //   "Trix3": -5, //triangle right corner x
  //   "Triy3": -50, //triangle right corner y

  // "arcx": -10, //x pos of arc
  // "arcy": -100, //y pos pf arc
  // "arcStart": 227.6, //start of arc
  // "arcStop": 72, //end of arc

  // "linex": -35, //top x of line
  // "liney": 45, //top y of line
  // "linex2": 40, //bottom x of line
  // "liney2": 45, //bottom y of line

  "offsetx": -5, //position x of ellipse
  "offsety": -100, //position y of ellipse

  "Trix1": -50, //triangle left corner x
  "Triy1": 40, //triangle left corner y
  "Trix2": 5, //triangle top corner x 
  "Triy2": -60, //triangle top corner y 
  "Trix3": -10, //triangle right corner x 
  "Triy3": 40, //triangle right corner y 

"arcx": -5, //x pos of arc
"arcy": -100, //y pos pf arc
"arcStart": 227.6, //start of arc
"arcStop": 52, //end of arc

"linex": 0, //top x of line
"liney": 38, //top y of line
"linex2": 40, //bottom x of line
"liney2": 38, //bottom y of line
   
  },
  "3": {

    "offsetx": 5, //25 //position x of ellipse
    "offsety": 0, //-130 //position y of ellipse

    "Trix1": -50, //triangle left corner x
    "Triy1": -140, //triangle left corner y
    "Trix2": 10, //triangle top corner x
    "Triy2": -60, //triangle top corner y
    "Trix3": 50, //triangle right corner x
    "Triy3": -140, //triangle right corner y

    "arcx": 5, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 256.4, //start of arc
    "arcStop": 140.4, //end of arc

    "linex": 57, //top x of line
    "liney": -135, //top y of line
    "linex2": 20, //bottom x of line
    "liney2": -60, //bottom y of line

  },
  "4": {
  
    "offsetx": 40, //position x of ellipse
    "offsety": -42, //position y of ellipse

    "Trix1": -45, //triangle left corner x
    "Triy1": -140, //triangle left corner y
    "Trix2": -45, //triangle top corner x
    "Triy2": -40, //triangle top corner y
    "Trix3": 10, //triangle right corner x
    "Triy3": -40, //triangle right corner y
    
    "arcx": 30, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 0,//-20, //start of arc
    "arcStop": 0,//-170, //end of arc

    "linex": 20, //top x of line
    "liney": -60, //top y of line
    "linex2": 20, //bottom x of line
    "liney2": 40, //bottom y of line

  },
  "5": {
   
    // "offsetx": 5, //25 //position x of ellipse
    // "offsety": 0, //-130 //position y of ellipse

    // "Trix1": -10, //triangle left corner x
    // "Triy1": -130, //triangle left corner y
    // "Trix2": -10, //triangle top corner x
    // "Triy2": -60, //triangle top corner y
    // "Trix3": 50, //triangle right corner x
    // "Triy3": -130, //triangle right corner y

    // "arcx": 5, //x pos of arc
    // "arcy": 0, //y pos pf arc
    // "arcStart": 256.4, //start of arc
    // "arcStop": 140.4, //end of arc

    // "linex": 0, //top x of line
    // "liney": -140, //top y of line
    // "linex2": 40, //bottom x of line
    // "liney2": -140, //bottom y of line

    //option one

    // "offsetx": 0, //25 //position x of ellipse
    // "offsety": 0, //-130 //position y of ellipse

    // "Trix1": -15, //triangle left corner x
    // "Triy1": -140, //triangle left corner y
    // "Trix2": -15, //triangle top corner x
    // "Triy2": -60, //triangle top corner y
    // "Trix3": 50, //triangle right corner x
    // "Triy3": -140, //triangle right corner y

    // "arcx": 0, //x pos of arc
    // "arcy": 0, //y pos pf arc
    // "arcStart": 256.4, //start of arc
    // "arcStop": 140.4, //end of arc

    // "linex": -25, //top x of line
    // "liney": -135, //top y of line
    // "linex2": -25, //bottom x of line
    // "liney2": -65, //bottom y of line

    //option 2

    // "offsetx": 0, //25 //position x of ellipse
    // "offsety": 0, //-130 //position y of ellipse

    // "Trix1": -20, //triangle left corner x
    // "Triy1": -140, //triangle left corner y
    // "Trix2": -20, //triangle top corner x
    // "Triy2": -100, //triangle top corner y
    // "Trix3": 50, //triangle right corner x
    // "Triy3": -140, //triangle right corner y

    // "arcx": 0, //x pos of arc
    // "arcy": 0, //y pos pf arc
    // "arcStart": 256.4, //start of arc
    // "arcStop": 140.4, //end of arc

    // "linex": -15, //top x of line
    // "liney": -90, //top y of line
    // "linex2": -15, //bottom x of line
    // "liney2": -50, //bottom y of line

    //option 3

    "offsetx": 0, //25 //position x of ellipse
    "offsety": 0, //-130 //position y of ellipse

    "Trix1": -20, //triangle left corner x
    "Triy1": -140, //triangle left corner y
    "Trix2": -20, //triangle top corner x
    "Triy2": -50, //triangle top corner y
    "Trix3": 0, //triangle right corner x
    "Triy3": -140, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 256.4, //start of arc
    "arcStop": 140.4, //end of arc

    "linex": 10, //top x of line
    "liney": -135, //top y of line
    "linex2": 50, //bottom x of line
    "liney2": -135, //bottom y of line

  },
  "6": {

    "offsetx": 0, //25 //position x of ellipse
    "offsety": 0, //-130 //position y of ellipse

    "Trix1": -10, //triangle left corner x
    "Triy1": -140, //triangle left corner y
    "Trix2": -30, //triangle top corner x
    "Triy2": -50, //triangle top corner y
    "Trix3": 30, //triangle right corner x
    "Triy3": -140, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos of arc
    "arcStart": 208, //start of arc
    "arcStop": 151, //stop of arc

    "linex": 40, //top x of line
    "liney": -135, //top y of line
    "linex2": -15, //bottom x of line
    "liney2": -50, //bottom y of line
   
  },
  "7": {

    "offsetx": 10, //25 //position x of ellipse
    "offsety": -120, //-130 //position y of ellipse

    "Trix1": -10, //triangle left corner x
    "Triy1":  40, //triangle left corner y
    "Trix2":  30, //triangle top corner x
    "Triy2": -130, //triangle top corner y
    "Trix3": 30, //triangle right corner x
    "Triy3": 40, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 0, //start of arc
    "arcStop": 0, //end of arc

    "linex": -30, //top x of line
    "liney": -140, //top y of line
    "linex2": 30, //bottom x of line
    "liney2": -140, //bottom y of line
    
  },
  "8": {
   
    "offsetx": 0, //position x of ellipse
    "offsety": -100, //position y of ellipse

    "Trix1": -30, //triangle left corner x
    "Triy1": -140, //triangle left corner y
    "Trix2": 0, //triangle top corner x
    "Triy2": -50, //triangle top corner y
    "Trix3": 30, //triangle right corner x
    "Triy3": -140, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 298.8, //start of arc
    "arcStop": 244.8, //end of arc

    "linex": 0, //top x of line
    "liney": -5, //top y of line
    "linex2": 0, //bottom x of line
    "liney2": 5, //bottom y of line

  },
  "9": {

    "offsetx": -10, //position x of ellipse
    "offsety": -100, //position y of ellipse

    "Trix1": 0, //triangle left corner x
    "Triy1": 40, //triangle left corner y
  "Trix2": 20, //triangle top corner x
  "Triy2": -140, //triangle top corner y
  "Trix3": 40, //triangle right corner x
  "Triy3": 40, //triangle right corner y

  "arcx": -5, //x pos of arc
  "arcy": -100, //y pos pf arc
  "arcStart": 61.2, //start of arc
  "arcStop": 298.8, //end of arc

  "linex": 30, //top x of line
  "liney": -138, //top y of line
  "linex2": 50, //bottom x of line
  "liney2": 38, //bottom y of line
    
  
  },

  "!": { 
   
    "offsetx": 5, //position x of ellipse
    "offsety": 35, //position y of ellipse

    "Trix1": 0, //triangle left corner x
    "Triy1": 0, //triangle left corner y
    "Trix2": 0, //triangle top corner x
    "Triy2": 0, //triangle top corner y
    "Trix3": 0, //triangle right corner x
    "Triy3": 0, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": 0, //y pos pf arc
    "arcStart": 0, //start of arc
  "arcStop": 0, //end of arc
    
    "linex":5, //top x of line
    "liney": -140, //top y of line
    "linex2": 5, //bottom x of line
    "liney2": 10, //bottom y of line

  },

  "?": { 
    
    "offsetx": 5, //position x of ellipse
    "offsety": 35, //position y of ellipse

    "Trix1": 0, //triangle left corner x
    "Triy1": 0, //triangle left corner y
    "Trix2": 0, //triangle top corner x
    "Triy2": 0, //triangle top corner y
    "Trix3": 0, //triangle right corner x
    "Triy3": 0, //triangle right corner y

    "arcx": 0, //x pos of arc
    "arcy": -100, //y pos pf arc
    "arcStart": 190, //start of arc
    "arcStop": 420, //end of arc
    
    "linex":5, //top x of line
    "liney": -50, //top y of line
    "linex2": 5, //bottom x of line
    "liney2": 10, //bottom y of line

  },
 
}
